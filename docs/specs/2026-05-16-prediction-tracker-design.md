...
**提交成功後的狀態頁（`/[locale]/submit` step === 'success'）**
...
[撤回這則提交]  ← 灰色文字按鈕，非主要 CTA
← 回首頁
```

點「撤回這則提交」：
1. 呼叫 `DELETE /api/predictions/[id]`
2. 成功 → 文字改為「已撤回」，按鈕消失
3. 失敗（非 pending_review）→ 顯示「審核已完成，無法撤回，請聯絡我們」

> 注意：撤回只在 `pending_review` 狀態有效。一旦 AI 審核通過（`status = active`），按鈕不顯示或顯示為 disabled。

### 我的提交紀錄（Plan 5）
登入後可在個人頁面（`/[locale]/me`）看到自己所有提交，含狀態（審核中 / 已上線 / 已刪除 / 已撤回）。`pending_review` 狀態的項目旁顯示「撤回」按鈕。
...
預言列表每一列有「刪除」按鈕，點擊後出現 inline confirm：

```
確定要刪除這則預言嗎？

刪除原因（必填）：[________________]

[取消]  [確認刪除]
```

確認後：
1. 呼叫 `DELETE /api/predictions/[id]`，body 帶 `{ reason: "..." }`
2. 成功 → 該列變灰，加上「已刪除」tag，顯示刪除者 email + 時間
3. 失敗 → 顯示 error message，不關閉 confirm

Admin 後台的預言列表預設**不顯示**已刪除的預言，可切換「顯示已刪除」filter 查看完整記錄（含刪除者、時間、原因）。
